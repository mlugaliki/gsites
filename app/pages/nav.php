<nav class="fix_topscroll logo_on_fixed topbar navigation">
<div class="nav-wrapper container">
    <a id="logo-container" href="http://guruhub.tech" class="brand-logo">GuruHub</a>

    <a href="#" data-target="" class="waves-effect waves-circle navicon back-button htmlmode show-on-large"><i
        class="mdi mdi-arrow-left" data-page=""></i></a>

    <a href="#" data-target="slide-nav" class="waves-effect waves-circle navicon sidenav-trigger show-on-large"><i
        class="mdi mdi-menu"></i></a>

    <a href="#" data-target="slide-settings"
    class="waves-effect waves-circle navicon right sidenav-trigger show-on-large"><i
        class="mdi mdi-settings-outline"></i></a>

    <a href="#" data-target="" class="waves-effect waves-circle navicon right nav-site-mode show-on-large"><i
        class="mdi mdi-invert-colors mdi-transition1"></i></a>
    <!-- <a href="#" data-target="nav-mobile" class="sidenav-trigger"><i class="material-icons">menu</i></a> -->
</div>
</nav>
<ul id="slide-nav" class="sidenav sidemenu">
<li class="user-wrap">
    <div class="user-view row">
    <div class="col s9 infoarea">
        <a href="ui-app-profile.html"><span class="name">Guruhub Innovators Ltd</span></a>
        <a href="ui-app-profile.html"><span class="email">info@guruhub.tech</span></a>
    </div>
    </div>
</li>
<li class="menulinks">
    <ul class="collapsible">
    <!-- Menu Data Start-->
    <li class="lvl1">
        <div class="waves-effect index">
        <a href="home.php?ver&&">
            <i class="mdi mdi-home-outline"></i>
            <span class="title">Home</span></a>
        </div>
    </li>
    <li class="lvl1">
        <div class="waves-effect index">
        <a href="product.php?name=30_day_yoga">
            <i class="mdi mdi-arrange-send-to-back"></i>
            <span class="title">Yoga</span></a>
        </div>
    </li>
    <li class="lvl1">
        <div class="waves-effect index">
        <a href="product.php?name=drumbell">
            <i class="mdi mdi-flask-outline"></i>
            <span class="title">Drumbell</span></a>
        </div>
    </li>
    <!-- <li class="lvl1">
        <div class="waves-effect index">
        <a href="home.php">
            <i class="mdi mdi-flask-outline"></i>
            <span class="title">Education</span></a>
        </div>
    </li>
    <li class="lvl1">
        <div class="waves-effect index">
        <a href="home.php">
            <i class="mdi mdi-flask-outline"></i>
            <span class="title">Kids stories</span></a>
        </div>
    </li>-->

    <li class="sep-wrap">
        <div class="divider"></div>
    </li>
    <li class="sh-wrap">
        <div class="subheader">Get in Touch</div>
    </li>
    <li class="lvl1">
        <div class="waves-effect">
        <a href="mailto:info@guruhub.tech">
            <i class="mdi mdi-email-outline"></i>
            <span class="title">Email</span>
        </a>
        </div>
    </li>
   <!-- <li class="lvl1">
        <div class="waves-effect">
        <a href="tel:+254774812586">
            <i class="mdi mdi-cellphone-basic"></i>
            <span class="title">Phone</span>
        </a>
        </div>
    </li>-->
    <li class="lvl1">
        <div class="waves-effect">
        <a href="sms:24697?&body=ball">
            <i class="mdi mdi-message-text-outline"></i>
            <span class="title">Message</span>
        </a>
        </div>
    </li>
    <!-- Menu Data End-->
    </ul>
</li>
</ul>
<div class="menu-close"><i class="mdi mdi-close"></i></div>
